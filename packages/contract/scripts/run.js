const main = async () => {
  const [randomPerson, randomPerson2, randomPerson3] =
  ...
  const contractBalanceAfterPost = await hre.ethers.provider.getBalance(
    postingContract.address,
  );
  console.log(
    'Contract balance:',
    hre.ethers.utils.formatEther(contractBalanceAfterPost),
  );

  // 投稿を確認
  const allPosts = await postingContract.getAllPosts();
  ...
  const allPost2 = await postingContract.getAllPosts();
  console.log(allPost2);

  // いいねの数を一つ減らしてみる
  await postingContract.connect(randomPerson).updateTotalLikes(0);

  const allPost3 = await postingContract.getAllPosts();
